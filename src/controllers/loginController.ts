...
class LoginController {
    ...
    public async loginUser(req: Request, res: Response): Promise<void> {
        ...
        const usuario = await db.query('SELECT * FROM usuarios Where email = ? or username = ?', [req.body.user,req.body.user]);
        if(usuario != ""){
    ...
    public async loginAdmin(req: Request, res: Response): Promise<void> {
        let password = req.body.password;
        try{
            const admin = await db.query('SELECT * FROM admin Where email = ? or username = ?', [req.body.user,req.body.user]);
            if(admin != ""){
                let savedPassword = admin[0].password;
                let match = await func.matchPassword(password, savedPassword);
                if(match){
                    let token = await func.getToken(req.body);
                    let tokenAdmin = await func.getTokenAdmin(req.body);
    ...
    public async loginEmpresa(req: Request, res: Response): Promise<void> {
        let password = req.body.password;
        const empresa = await db.query('SELECT * FROM empresas Where email = ? or username = ?', [req.body.user,req.body.user]);
        if(empresa != ""){
            let savedPassword = empresa[0].password;
            let match = await func.matchPassword(password, savedPassword);
            if(match){
                let token = await func.getToken(req.body);
                let tokenEmpresa = await func.getTokenEmpresa(req.body);
    ...
    public async updatePasswordNosotros(req: Request, res: Response): Promise<void> {
        let password = req.body.password;
        const { id } = req.params;
        req.body.password = await func.encryptPassword(password);
        try{
            await db.query('UPDATE admin set ? WHERE id = ?', [req.body, id]);
            res.json({ message: "ok" });
        }catch(err){
